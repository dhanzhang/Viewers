...
const dblClickOnStudy = data => {
    ...
    OHIF.measurements.createTimepointForUnicStudyInstanceUID(data);
    const timepoint = timepointApi.study(data.studyInstanceUid)[0];
    if (timepoint) {
        Router.go('viewerStudies', { studyInstanceUids: data.studyInstanceUid, timepointId: timepoint.timepointId });
    }
};
